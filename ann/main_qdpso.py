"""
Benchmark: Entrenamiento de Redes Neuronales con QDPSO

Este script ejecuta un benchmark comparativo usando QDPSO en los datasets:
- Iris (4 features, 3 clases)
- Wine (13 features, 3 clases)
- Breast Cancer (30 features, 2 clases)

Configuracion:
- Optimizador: QDPSO (Quantum Delta PSO)
...
from optimizers import QDPSONNOptimizer  # QDPSO en lugar de QPSO
...
IMG_OUTPUT_DIR = './img/metric/QDPSO'
...
def get_image_filename(
    ...
    filename = f"QDPSO_{dataset}_{plot_type}_{g_str}_{particles}_{iters}_{timestamp}.png"
    return filename
...
def run_benchmark(
    ...
    """
    Ejecuta benchmark en un dataset especifico usando QDPSO.
    ...
    """
    print_header(f"BENCHMARK QDPSO: {dataset_name.upper()}")
    ...
    # ==========================================================================
    # 4. Configurar optimizador QDPSO
    # ==========================================================================

    print_subheader("4. Configurando optimizador QDPSO")

    opt_config = NNOptimizationConfig(
        n_particles=config['n_particles'],
        max_iters=config['max_iters'],
        g=config['g'],  # Factor g para QDPSO
    ...
    print_subheader("5. Entrenando modelo con QDPSO")
    ...
    print(f"  Dataset: {dataset_name}")
    print(f"  Optimizador: QDPSO (g={config['g']})")
    ...
    if history and 'train_loss' in history:
        ...
        plot_loss_curves(
            history=history,
            title=f"QDPSO - Curvas de Loss ({dataset_name})",
            save_path=loss_path
        )
    ...
    if history and 'train_acc' in history:
        ...
        plot_accuracy_curves(
            history=history,
            title=f"QDPSO - Curvas de Accuracy ({dataset_name})",
            save_path=acc_path
        )
    ...
    plot_complete_training_summary(
        ...
        title=f"QDPSO - Resumen Entrenamiento ({dataset_name})",
    ...
    if cv_result.fold_results:
        ...
        plot_cv_summary(
            ...
            title=f"QDPSO - Cross-Validation ({dataset_name}, {config['n_folds']} folds)",
            save_path=cv_path
        )
...
def main():
    """Ejecuta el benchmark completo con QDPSO."""
    ...
    # Configuracion
    print_header("CONFIGURACION DEL BENCHMARK QDPSO")
    ...
    print_header("RESUMEN FINAL DEL BENCHMARK QDPSO", "=")

    print(f"\nFecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Dispositivo: {device}")
    print(f"Optimizador: QDPSO (g={BENCHMARK_CONFIG['g']})")
    ...
    print("\n" + "=" * 90)
    print(" BENCHMARK QDPSO COMPLETADO")
    print("=" * 90)
