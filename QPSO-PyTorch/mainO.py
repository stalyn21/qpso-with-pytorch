# kernel_update() mejorado y el mejro tiempo de ejecución --Last versión
import torch
...
def sphere(args):
    return torch.sum(args ** 2, dim=1)  # Evaluamos la suma a lo largo de las dimensiones
...
def log(s):
    ...
    best_value_avg = torch.mean(best_values)
    best_value_std = torch.std(best_values)
    
    # Imprimimos los resultados
    print("{0: >5}  {1: >9}  {2: >9}  {3: >9}".format("Iters.", "Best", "Best(Mean)", "Best(STD)"))
    print("{0: >5}  {1: >9.3E}  {2: >9.3E}  {3: >9.3E}".format(s.iters, s.gbest_value.item(), best_value_avg.item(), best_value_std.item()))

# Parámetros de la optimización
NParticle = 40
MaxIters = 100
NDim = 10
bounds = [(-2.56, 5.12) for _ in range(NDim)]
g = 0.96
