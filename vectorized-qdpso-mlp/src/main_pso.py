...
def main():
    ...
    args = parse_args('PSO / PSO_bound (pyswarms-style, implementación torch)',
                      pso_variant=True)
    set_seeds(args.seed)
    ensure_dirs(args.variant)
    ...
    config = {
        'optimizer': args.variant, # options: PSO_bound, PSO
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train_val), 1):
        ...
        # PSO_bound usa límites (-1, 1); PSO corre sin límites
        if args.variant == 'PSO_bound':
    ...
    results_path = dump_results(args.variant, args, config, train_results,
