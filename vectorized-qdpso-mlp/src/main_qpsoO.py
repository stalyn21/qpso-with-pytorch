...
from one_swarm.ann.ann_qpsoO import ExtendedModel
...
def main():
    args = parse_args('QDPSOo (implementación tensor-wise optimizada)')
    set_seeds(args.seed)
    ensure_dirs("QPSOo")
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train_val), 1):
        ...
        with torch.no_grad():
            ...
            score = select_score(val_acc, accuracy, args.select_by)
            if score > best_acc:
                best_acc = score
    ...
    results_path = dump_results("QPSOo", args, config, train_results,
