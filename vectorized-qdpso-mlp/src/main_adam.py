...
from run_utils import (parse_args, set_seeds, load_data, run_suffix,
                       ensure_dirs, select_score, dump_results)
...
def main():
    ...
    args = parse_args('Adam (PyTorch, baseline gradiente)', adam_arch=True)
    set_seeds(args.seed)
    ensure_dirs("Adam")

    dataset_name = args.dataset

    X_trainval, X_test, y_trainval, y_test = load_data(args)

    input_shape = X_trainval.shape[1]
    output_shape = len(np.unique(y_trainval))
    n_samples = X_trainval.shape[0]

    # Por defecto se usa la MISMA arquitectura que los metaheurísticos
    # [3in, 2in, in]; --legacy-arch reproduce la original del paper (más chica)
    if args.legacy_arch:
    ...
    config = {
        ...
        'features_reduction': run_suffix(args, input_shape),
        "lr": 0.001,
    ...
    kf = KFold(n_splits=config["n_folds"], shuffle=True, random_state=args.split_seed)
    train_results = []
    val_results = []
    test_results = []
    time_results = []
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_trainval)):
        ...
        # Selección del mejor modelo entre folds por VALIDACIÓN (no por test)
        score = select_score(val_acc, test_acc, args.select_by)
        if score > best_acc:
            best_acc = score
    ...
    logging.info(f'Mean training time per fold: {mean_time:.4f} - std:{std_time:.4f} seconds')
    ...
    results_path = dump_results("Adam", args, config, train_results,
                                val_results, test_results, time_results)
    logging.info(f"Per-fold results saved at: {results_path}")
