...
class WeatherImageDataset:
    ...
    def load_and_preprocess_data(self, test_size=0.2, random_state=100):
        ...
        # Caché de features extraídas: la extracción (HSV+Haralick+Hu sobre ~1120
        # imágenes) es determinista dado img_size y bins, así que se calcula una
        # sola vez y se reutiliza entre runs
        cache_path = (self.root_path.parent /
                      f"mcw_features_{self.img_size[0]}x{self.img_size[1]}_b{self.bins}.npz")
        if cache_path.exists():
            print(f"Cargando features desde caché: {cache_path}")
            cached = np.load(cache_path)
            X, y = cached['X'], cached['y']
            return self._split_dataset(X, y, test_size, random_state)
    ...
    def _split_dataset(self, X, y, test_size, random_state):
