    ...
    def kernel_update(self, **kwargs):
        for p in self._particles:
            for i in range(self._dim):
                u1 = torch.tensor(random.uniform(0., 1.), device=device)
                u2 = torch.tensor(random.uniform(0., 1.), device=device)
                u3 = torch.tensor(random.uniform(0., 1.), device=device)
                rand_sign = 1 if random.random() > 0.5 else -1
                c = (u1 * p.best[i] + u2 * self._gbest[i]) / (u1 + u2)
                L = (1 / self._g) * abs(p[i] - c)
                p[i] = c + rand_sign * L * torch.log(torch.tensor(1. / u3, device=device))
