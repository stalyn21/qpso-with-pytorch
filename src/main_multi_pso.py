...
from multi_swarm.custome_optimizer.pso_optimizer import LayerPSOOptimizer
from multi_swarm.ann.model_pso import ExtendedModelPSO
...
def main():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train_val), 1):
        ...
        swarm_times = np.zeros(len(model.layers))

        # Entrenamiento por capas
        training_start_time = time.perf_counter()
        for layer_idx, optimizer in enumerate(swarms):
            ...
            swarm_times[layer_idx] = layer_end_time - layer_start_time
        ...
        time_results.append(training_end_time - training_start_time)
    ...
    # Calcular y mostrar tiempos medios por capa
    mean_layer_times = np.mean(swarm_times)
    std_layer_times = np.std(swarm_times)
    for layer_idx in range(len(model.layers)):
        logging.info(f"Mean time per layer {layer_idx}: {mean_layer_times:.4f} - std: {std_layer_times:.4f} seconds")
